...
if "current_section_index" not in st.session_state:  # Changed from current_section_idx
    st.session_state.current_section_index = 0     # Changed from current_section_idx
...
if "feedback" not in st.session_state: # Initialize feedback dict
...
if st.sidebar.button("Generate Course"):
    if uploaded_file or pdf_url:
        ...
        st.session_state.current_section_index = 0 # Changed from current_section_idx
        reset_section_attempt_state() # Reset answers and checked status        proceed_with_generation = True
