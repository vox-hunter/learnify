...
def display_question(question_item, section_key, question_idx):
    ...
    if question_type in ["multiple_choice", "multiple choice"]:
        ...
        correct_answer_for_blank = str(question_item.get("answer", ""))
        ...
        # Check if the question_text_full contains underscores (e.g., '___')
            ...
            current_blank_value = st.session_state[question_key]            user_input_for_blank = fill_in_the_blanks_input(
            ...
            if user_input_for_blank != current_blank_value:
                ...
                handle_answer_submission(question_key, correct_answer_for_blank, question_item.get('type'))
                st.rerun()
                
    elif question_type == "match":
        # Get the matching items from the question's answer
        match_data = question_item.get('answer', {})
        ...
        if isinstance(match_data, list):
            try:
                for item in match_data:
                    if isinstance(item, dict) and "premise" in item and "response" in item:
                        match_dict[item["premise"]] = item["response"]
                    # Handle case where items are in a nested structure
                    elif isinstance(item, dict) and len(item) == 1:
                        key = list(item.keys())[0]
                        inner_item = item[key]
                        if isinstance(inner_item, dict) and "premise" in inner_item and "response" in inner_item:
        ...
        if isinstance(match_data, dict) and match_data:
            ...
            with match_container:
                ...
                for i, left_item in enumerate(left_items):
                    with st.container():
                        ...
                        with cols[0]:
                            st.markdown(
                                f"""
                                <div style="background-color: #f0f2f6; padding: 10px; border-radius: 5px; min-height: 50px; display: flex; align-items: center;">
                                    <span>{left_item}</span>
                                </div>
                                """, 
                ...
                if is_answered:
                    ...
                    for left_item, user_right_item in user_matches.items():
                        ...
                        with cols[2]:
                            bg_color = "#d1ffd6" if is_match_correct else "#ffe0e0"  # Green if correct, red if incorrect
                            st.markdown(
                                f"""
                                <div style="background-color: {bg_color}; padding: 10px; border-radius: 5px; min-height: 50px; display: flex; align-items: center;">
                                    <span>{user_right_item}</span>
                                </div>
                                """, 
                                unsafe_allow_html=True
                            )
            ...
            st.json(match_data)  # Show the raw data for debugging
    elif question_type in ["short_answer", "short answer"]:
        st.text_area("Your answer:",
                    key=question_key,
                    on_change=handle_answer_submission,
                    args=(question_key, question_item.get('answer'), question_item.get('type'), None),
                    disabled=is_answered
                    )
...
def handle_answer_submission(question_key, correct_answer, question_type, placeholder_option_value=None):
    ...
    # Revert short_answer to simple string comparison
        ...
        if str(user_answer).strip().lower() == str(correct_answer).strip().lower():
            is_correct_locally = True
        feedback_message = f"Your answer: {user_answer}, Expected: {correct_answer}"    # For other text-based answers (multiple_choice, fill_in_the_blank, match)
    elif question_type == "match":
