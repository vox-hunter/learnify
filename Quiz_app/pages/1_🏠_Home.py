...
def main():
    ...
    if just_logged_out:
        ...
        st.session_state.pop('logout_just_occurred_processed_auto_login_home', None)        # Attempt auto-login only if cookies are ready and not immediately after a logout action
        if cookies:
            try:
                if cookies.ready():
    ...
    with col2: # Login/User status
        ...
        elif MONGO_AVAILABLE and cookies:
            try:
                if cookies.ready():
