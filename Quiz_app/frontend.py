...
MONGO_AVAILABLE = MongoAuthManager is not None

# Background warm-up (ignore failures)
prefetch_modules([
    "mongo_auth",
    "local_backend",
...
if not _cookies_ready() and EncryptedCookieManager is not None:
...
def initialize_session_state():
    ...
    if "pending_pdf_url" not in st.session_state:
        st.session_state.pending_pdf_url = None    # Authentication and course limit tracking
...
def display_question(question_item, section_key, question_idx):    # Check if question_item is a Pydantic model
    ...
    if question_type in ["multiple_choice", "multiple choice"]:
        ...
        # Check if the question_text_full contains underscores (e.g., '___')
            ...
            if fill_in_the_blanks_input is not None:
                ...
                user_input_for_blank = st.text_input("Your answer:", key=question_key, value=current_blank_value, disabled=is_answered)
...
def main():
    ...
    if not st.session_state.authentication_status:
        ...
        courses_used = get_guest_course_count()
            ...
            try:
                ...
                if course_data:
                    ...
                    if st.session_state.authentication_status:
                        ...
                        remaining = 3 - get_guest_course_count()
                        if remaining > 0:
...
# Guest course tracking constants
